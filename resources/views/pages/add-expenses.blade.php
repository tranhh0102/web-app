    ...
    <div class="add-expenses">
        ...
        <div class="add-expenses-sub flex" id="openModal">
            <div class="flex gap-2">
                <span class="add-expenses-span" id="selectedType">Loại</span>
            </div>
            <!-- <img width="24" height="24" src="{{ asset('svg/arrow.svg') }}" alt=""> -->
        </div>
...
<script>
    ...
    // Khi nhấn vào "Loại" → Hiện modal
    openModal.addEventListener("click", function () {
        modalOverlay.style.display = "block";
        modalSheet.classList.add("show");
    });

    // Khi chọn một loại → Cập nhật và đóng modal
    modalItems.forEach(item => {
        item.addEventListener("click", function () {
            selectedType.textContent = this.getAttribute("data-value");
            closeModalFunc();
        });
    });

    // Khi nhấn "Đóng" → Ẩn modal
    closeModal.addEventListener("click", closeModalFunc);

    // Khi nhấn ra ngoài → Ẩn modal
